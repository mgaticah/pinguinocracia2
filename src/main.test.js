...
vi.mock('./systems/MapManager.js', () => ({ default: class { constructor () { this._obstacleGroup = null }; loadMap (key, scene) { if (scene && scene.physics && scene.physics.add) this._obstacleGroup = { type: 'staticGroup' }; return { key } }; getMapDimensions () { return { width: 3840, height: 2160 } }; getEntryPoint () { return { x: 192, y: 1080 } }; getObstacleLayer () { return this._obstacleGroup }; getPowerupPoints () { return [{ x: 960, y: 540 }] }; getExitZones (key) { if (key === 'map_level1') return [{ x: 3792, y: 960, width: 48, height: 192, targetMap: 'map_level2' }]; return [] }; getSpawnPoints () { return [{ x: 96, y: 96 }] } } }))
...
describe('Complete game flow', () => {
  ...
  describe('4: map transition', () => {
    ...
    it('emits map:transition', async () => { const EB = (await import('./EventBus.js')).default; EB.emit.mockClear(); gameScene._transitionToMap('map_level2'); expect(EB.emit).toHaveBeenCalledWith('map:transition', { mapKey: 'map_level2' }) })
    it('clears groups', () => { gameScene._isTransitioning = false; gameScene._transitionToMap('map_level2'); gameScene._proceedTransition('map_level2'); dc(); expect(gameScene.enemyGroup.clear).toHaveBeenCalledWith(true, true); expect(gameScene.powerupGroup.clear).toHaveBeenCalledWith(true, true); expect(gameScene.projectileGroup.clear).toHaveBeenCalledWith(true, true) })
    it('preserves state', () => { gameScene.player.hp = 7; gameScene.globalCounter.molotovs = 3; gameScene.scoreSystem.score = 500; gameScene.totalTime = 180; gameScene._isTransitioning = false; gameScene._transitionToMap('map_level2'); gameScene._proceedTransition('map_level2'); dc(); expect(gameScene.player.hp).toBe(7); expect(gameScene.globalCounter.molotovs).toBe(3); expect(gameScene.scoreSystem.score).toBe(500); expect(gameScene.totalTime).toBe(180) })
    it('grants ally', () => { gameScene._isTransitioning = false; gameScene._transitionToMap('map_level2'); gameScene._proceedTransition('map_level2'); dc(); expect(gameScene.allyGroup.add).toHaveBeenCalled() })
  })
